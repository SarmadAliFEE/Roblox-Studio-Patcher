...
enum LuauBytecodeTag
{
    ...
    LBC_VERSION_MIN = 3,
    LBC_VERSION_MAX = 13,
    // Patched from the upstream default (9) to 13 - Roblox Studio's own
    // loader was confirmed (empirically, via a live call into it) to accept
    // version 13 bytecode, so compiling to the version already proven to
    // work avoids depending on any FastFlag-gated version bump.
    LBC_VERSION_TARGET = 13,
