...
mod arch {
    pub const STEP: &str = concat!(
        ...
        "a8 00 00 37 88 c6 40 f9 08 c1 1b 91 08 fd df 08 ",
    ...
    pub const LUAU_LOAD_WRAPPER: &str = concat!(
        ...
        "18 14 40 f9 19 23 40 a9 1f 01 19 eb c3 00 00 54",
    );

    pub const CALL_DISPATCH: &str = concat!(
        "f6 57 bd a9 f4 4f 01 a9 fd 7b 02 a9 fd 83 00 91 ",
        "f4 03 02 aa f3 03 00 aa ?? ?? ?? ?? f5 03 00 aa ",
        "60 02 00 35 68 16 40 f9 08 99 42 f9 a8 02 00 b5 ",
        "75 12 40 79 68 0e 40 f9 02 d1 34 cb ?? ?? ?? ??",
    );
    ...
    pub const LUA_NEWTHREAD: &str = concat!(
        ...
        "08 14 40 f9 08 25 40 a9 3f 01 08 eb 83 00 00 54 ",
...
mod arch {
    pub const STEP: &str = concat!(
        ...
        "0f b6 80 80 07 00 00",
    );

    pub const LUAU_LOAD_WRAPPER: &str = concat!(
        "48 89 5c 24 08 48 89 6c 24 10 48 89 74 24 18 57 ",
        "41 56 41 57 48 81 ec 80 00 00 00 49 8b e9 4d 8b ",
        "f0 4c 8b fa 48 8b f9 48 8b 59 28 48 8b 33 48 39 ",
        "73 08 72 0e b2 01 e8 ?? ?? ?? ??",
    );

    pub const CALL_DISPATCH: &str = concat!(
        "48 89 74 24 10 57 48 83 ec 20 49 63 f0 48 8b f9 ",
        "44 8b c6 e8 ?? ?? ?? ?? 85 c0 75 66 48 8b 47 28 ",
        "48 89 5c 24 30 48 8b 90 30 05 00 00 48 85 d2 74 ",
        "05 48 8b cf ff d2 4c 8b 47 18",
    );

    pub const TASK_DEFER: &str = "";
    pub const LUA_NEWTHREAD: &str = concat!(
        "40 53 48 83 ec 20 48 8b 51 28 48 8b d9 48 8b 02 ",
        "48 39 42 08 72 07 b2 01 e8 ?? ?? ?? ?? f6 43 01 ",
        "04 74",
    );
    pub const CAN_ACCESS_RESTRICTED: &str = concat!(
